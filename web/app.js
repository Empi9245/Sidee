(() => {
  ...
  function pageContext(){ return {href:location.href,origin:location.origin,protocol:location.protocol,hostname:location.hostname,port:location.port||("https:"===location.protocol?"443":"http:"===location.protocol?"80":""),host:location.host,secureContext:window.isSecureContext===true}; }
  ...
  async function load(){
    ...
    state.config=d;state.report.accessContext=pageContext();state.report.serverAccess={host:status.host||null,requestScheme:status.requestScheme||null,requestPort:status.requestPort||null};state.report.serverBuildId=status.clientBuildId||null;state.report.buildMatch=!!status.clientBuildId&&status.clientBuildId===CLIENT_BUILD_ID;
